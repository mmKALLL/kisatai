...
import {
  handlePlayerMove,
  handlePlayerJump,
  handlePlayerFastFall,
  handlePlayerSDI,
} from './physics'

function inputHeld(player: Player, inputs: InputStatus, inputToCheck: PlayerInput) {
  return player.playerInputs[inputToCheck].some(
    (keyName) => inputs[keyName] && inputs[keyName].isDown
  )
}

export const handlePlayerInputs = (
  currentState: InGameState,
  inputs: InputStatus,
  keysPressed: KeyStatus[],
  keysReleased: KeyStatus[]
): InGameState => {
  ...
  nextPlayers.forEach((player) => {
    ...
    if (
      inputHeld(player, inputs, PlayerInput.Right) &&
      !inputHeld(player, inputs, PlayerInput.Left)
    ) {
      ...
      if (playerCanSDI(player) && isDirectionalInput(input)) {
        ...
        switch (input) {
          ...
          case PlayerInput.Special:
            nextState.activeAttacks = handleAttack(
              'Special',
              player,
              inputs,
              nextState.activeAttacks
            )
            break
...
function inputToAttackDirection(
  ...
  switch (input) {
    ...
    case PlayerInput.Left:
      return state === 'groundborne' ? 'Forward' : facing === 'left' ? 'Forward' : 'Back'
    case PlayerInput.Right:
      return state === 'groundborne' ? 'Forward' : facing === 'right' ? 'Forward' : 'Back'
    case PlayerInput.Down:
      return 'Down'
    case PlayerInput.Up:
      return state === 'airborne' ? 'Up' : 'Neutral'
    case PlayerInput.Neutral:
      return 'Neutral'
    case undefined:
      return 'Neutral'
    default:
...
function addActiveAttack(
  ...
  const attackX =
    attack.createUsingWorldCoordinates || attack.movesWithPlayer
      ? attack.x
      : player.x + attack.x * xDirection
  const attackY =
    attack.createUsingWorldCoordinates || attack.movesWithPlayer ? attack.y : player.y + attack.y

  const newHitboxes = attack.hitboxes.slice().map((hitbox) => ({
    ...hitbox,
  ...
  keysPressed.forEach((key: KeyStatus) => {
    switch (key.keyName) {
      ...
      case 'c':
        ...
        break
      case 'v':
        if (!nextState.start) nextState.playerReady[0] = false
        break
      case 'ArrowUp':
      ...
      case ',':
        ...
        break
      case '.':
        if (!nextState.start) nextState.playerReady[1] = false
        break
    }
  })
